...
class AboutIteration < Neo::Koan
  ...
  def test_inject_will_blow_your_mind
    # so inject is like reduce? the value passed in is the sum, and the second is each item in the array
    result = [2, 3, 4].inject(0) { |sum, item| sum + item }
    assert_equal 9, result

    result = [2, 3, 4].inject([]) { |sum, item| sum << item + 1 }
    ...
    # inject iterates over each item from an array, and runs a function defined in a block over them.
    # with the initial value passed in
  end
  ...
  #   # code to read 'file'
  #
  # its safer to have all the file related code in one block
  #
